...
// Stylowanie parametrów :class="paddingClasses[padding]"
// const props = withDefaults(defineProps<{ padding?: 'sm' | 'md' | 'lg' }>(), { padding: 'md' });
    ...
    <div class="mx-auto my-5 w-full max-w-sm font-sans">
        <div ref="containerRef" class="relative w-full rounded-lg border-2 border-blue-500 p-4 outline-none">
            ...
            <input
                ...
                class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm transition outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                :style="{
            ...
            <ul v-if="isExpanded && filteredCategories.length > 0" :id="listboxId" role="listbox" class="absolute right-4 left-4 z-10 mt-1 max-h-40 overflow-y-auto rounded-md border border-gray-200 bg-white p-0 shadow-lg">
