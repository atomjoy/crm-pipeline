...
const filteredCategories = computed<Category[]>(() => {
    return allCategories.value.filter((category) => {
        ...
        const isNotSelected = !selectedCategories.value.some((selected) => selected.id === category.id);
...
<template>
    <div class="multiselect-container" style="font-family: sans-serif; max-width: 400px; position: relative">
        <input type="text" role="combobox" v-model="searchQuery" @input="handleInput" @keydown="handleKeydown" @focus="isExpanded = true" @blur="setTimeout(() => (isExpanded = false), 200)" autocomplete="off" aria-autocomplete="list" :aria-expanded="isExpanded" :aria-controls="listboxId" :aria-activedescendant="activeDescendantId" placeholder="Wyszukaj i dodaj kategorię..." style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 6px; box-sizing: border-box; outline: none; font-size: 14px" />
