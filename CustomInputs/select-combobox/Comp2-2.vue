...
import { ref, computed, useId, watch, nextTick } from 'vue'; // Dodano watch i nextTick
...
// --- NOWOŚĆ: Referencje do elementów DOM ---
const itemRefs = ref<HTMLElement[]>([]);

// Czyszczenie tablicy referencji przed każdym ponownym renderowaniem pętli v-for
...
// --- NOWOŚĆ: Watcher przewijający listę ---
watch(focusedIndex, async (newIndex) => {
    // Czekamy na aktualizację DOM przez Vue
    await nextTick();

    const activeElement = itemRefs.value[newIndex];
    if (activeElement) {
        activeElement.scrollIntoView({
            block: 'nearest', // Przewija tylko tyle, ile potrzeba, by element stał się widoczny
            behavior: 'smooth', // Nadaje płynny efekt animacji (możesz zmienić na 'auto' dla natychmiastowego efektu)
        });
    }
});
...
// Resetowanie tablicy referencji i indeksu przy filtrowaniu
